...
function loadYAML() {
    fetch('car-rental.yaml')  // Path to your YAML file
        ...
        .then(yamlText => {
            ...
            // Render only the first 3 cars
            renderCarCards(carData, 3);  // Pass the limit (3) to the render function
        })
        .catch(error => {
...
// Function to render car cards (with a limit on how many cars to show)
function renderCarCards(carData, limit) {
    const carCardsContainer = document.getElementById('carCards');
    carCardsContainer.innerHTML = ''; // Clear the container

    // Loop through each car and create a card for each one, but only up to the specified limit
    const carsToRender = carData.cars.slice(0, limit); // Limit the number of cars rendered to the specified value
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="booking-opt">
                            <a href="${car.booking_options.message_phone}" class="btn btn-primary">
                                <i class="fas fa-phone" style="font-size: 23px;"></i>
                            </a>
